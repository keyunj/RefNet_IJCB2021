...
def run_epoch(
    data_loader, net, use_gt=False, phase="train", optimizer=None, criterian=None, epoch=0, args=None, tb_writer=None
):
    ...
    for batch_idx, (batch_sample, batch_name) in enumerate(data_loader):
        ...
        if phase == "train":
            ...
            ori_pred, seg_pred, type_pred, delta_pred_lst, anchors_ang = net(batch_sample, use_gt)
        else:
            with torch.no_grad():
                ori_pred, seg_pred, type_pred, delta_pred_lst, anchors_ang = net(batch_sample)
        ...
        for ii in range(args.n_anchors):
            # for loss function
            ori_delta = minus_orientation(batch_sample["ori_ang"], anchors_ang[:, ii : ii + 1])
            ori_delta = binary_orientation(ori_delta, args.ori_stride)
            loss_ori += criterian["ori"][1](
                delta_pred_lst[ii],
                ori_delta.detach(),
                batch_sample["seg"],
                batch_sample["soft_bin"][:, ii].view(-1, 1, 1, 1),
                logits=False,
            )
        ori_bin = binary_orientation(batch_sample["ori_ang"], args.ori_stride)
        loss_ori += criterian["ori"][1](ori_pred, ori_bin.detach(), batch_sample["seg"], logits=False)
        loss_ori = criterian["ori"][0] * loss_ori / 2
        ...
        type_acc = accuracy(type_pred, batch_sample["type"])[0]
        ...
        if phase == "train" and tb_writer is not None and batch_idx % 500 == 0:
            # scalar
            ...
            H = anchors_ang.size(-2)
            W = anchors_ang.size(-1)
            anchor_ang = anchors_ang.gather(1, torch.argmax(type_pred, dim=1).view(-1, 1, 1, 1).expand(-1, -1, H, W))
            ...
            tb_writer.add_figure(
                ...
                draw_on_image(
                    ...
                    seg_iou,
                    batch_name,
                    type_pred=type_pred.detach(),
                    type_true=batch_sample["type"],
                ),
                epoch * batch_N + batch_idx,
            )

        # plot progress
        suffix = f"Td+b:{data_time.avg + batch_time.avg:.3f} | "
        suffix += f"Tt:{bar.elapsed_td.total_seconds()//60} | ETA:{bar.eta_td.total_seconds()//60} | "
        suffix += f"Lori:{losses_ori.avg:.3f} Lseg:{losses_seg.avg:.3f} Ltype:{losses_type.avg:.3f} | "
        suffix += f"IOU:{seg_ious.avg*100:.2f} Angle:{ori_deltas.avg:.2f} Type:{type_accs.avg:.2f}"
...
if __name__ == "__main__":
    ...
    train_set = dataset.MultiDataSet(args, "train", transforms=train_transform)
    valid_set = dataset.MultiDataSet(args, "valid", transforms=None)
    ...
    for k, v in args.loss_lst.items():
        if k == "wseg":
            ...
            criterian["ori"] = [
                v,
                WeightedOriLoss(ang_stride=args.ori_stride, use_cuda=args.use_cuda),
            ]
        elif k == "fori":
            criterian["ori"] = [
                v,
                FocalOriLoss(ang_stride=args.ori_stride, use_cuda=args.use_cuda),
            ]
        elif k == "wtype":
            criterian["type"] = [v, nn.CrossEntropyLoss()]
        elif k == "ftype":
            criterian["type"] = [v, FocalTypeLoss(use_cuda=args.use_cuda)]
        else:
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        ...
        val_loss, val_ori_delta, val_seg_iou, val_type_acc = run_epoch(
            valid_loader, net, use_gt, phase="valid", criterian=criterian, epoch=epoch, args=args, tb_writer=tb_writer
        )
