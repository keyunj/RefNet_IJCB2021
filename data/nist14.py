...
def save_jsons(train_samples, valid_samples, test_samples, prefix, args, outdir):
    ...
    outdir = f"{outdir}4{args.pose_type}"
...
def random_split_dataset(img_path_lst, img_type_lst, prefix, args):
    ...
    if args.use_all:
        ...
        train_samples = [[img_path_lst[ii], img_type_lst[ii]] for ii in indices[:train_N]]
        valid_samples = [[img_path_lst[ii], img_type_lst[ii]] for ii in indices[-valid_N:]]
        test_samples = []
    else:
        valid_N = len(indices) // 5
        train_N = len(indices) - 2 * valid_N
        train_samples = [[img_path_lst[ii], img_type_lst[ii]] for ii in indices[:train_N]]
        valid_samples = [[img_path_lst[ii], img_type_lst[ii]] for ii in indices[train_N : train_N + valid_N]]
        test_samples = [[img_path_lst[ii], img_type_lst[ii]] for ii in indices[-valid_N:]]
