...
from ops.modules import expectation_pred, EnhanceModule, transform_angle, transform_anchor
...
def run_on_image(img, anchors_ang, anchors_rmsd, pose, args, net, enhance):
    ...
    with torch.no_grad():
        ori_pred, seg_pred, anchors_ang = net(sample)
        # enhancing
        ori_bin = transform_angle(ori_pred, anchors_ang, args.ori_stride)
        sample["data"] = enhance(sample["data"], ori_bin)
        ori_pred, _, _ = net(sample)
...
def deploy(args):
    ...
    for data_idx, img_path in enumerate(img_path_lst):
        ...
        ori_angle, seg_pred = run_on_image(img, anchors_ang, anchors_rmsd, pose, args, net, enhance)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pose-dir", default="manual", type=str)
