...
def run_on_image(img, anchors_ang, anchors_rmsd, pose, args, net):
    ...
    if len(pose) > 3:
        ...
        pose_scale = np.ones_like(pose_trans)
    ...
    batch_sample["pose_scale"] = torch.tensor(pose_scale[None].astype(np.float32))
    if args.use_cuda:
        ...
        batch_sample["pose_scale"] = batch_sample["pose_scale"].cuda()
...
def deploy(args):
        ...
        np.stack(
            [
                np.asarray(
                    imageio.imread(
                        osp.join(
                            "data", "anchors", args.pose_type.split("-")[0], f"{args.n_anchors}", f"meanfield{x}.png"
                        )
                    ),
                    np.float32,
                )
    ...
    anchors_rmsd = np.stack(
        [
            np.asarray(
                imageio.imread(
                    osp.join("data", "anchors", args.pose_type.split("-")[0], f"{args.n_anchors}", f"rmsd{x}.png")
                ),
                np.float32,
            )
            for x in range(args.n_anchors)
    ...
    for data_idx, img_path in enumerate(img_path_lst):
        ...
        type_thresh = 0
        for ii in range(args.n_anchors):
            ori_angle, seg_pred, type_pred = run_on_image(img, anchors_ang[ii], anchors_rmsd[ii], pose, args, nets[ii])
        ...
        type_cls.append(np.argmax(type_lst, axis=0))
        ...
        imageio.imwrite(osp.join(tar_ori_dir, f"{eval_name}.png"), np.rint(ori_angle + 90).astype(np.uint8))
        imageio.imwrite(osp.join(tar_seg_dir, f"{eval_name}.png"), np.rint(seg_pred * 255).astype(np.uint8))
...
if __name__ == "__main__":
    ...
    args.output = osp.join(osp.dirname(args.deploy_set), "estimation", logdir, args.pose_dir)
